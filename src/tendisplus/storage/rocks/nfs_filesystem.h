...
class NFSFileSystem : public FileSystemWrapper {
 public:
  static const char* kClassName() { return "NFSFileSystem"; }
  static const char* kNickName() { return "nfs"; }
  ...
  const std::string& GetNFSUrl() const { return nfs_url_; }
  const std::string& GetLocalPrefix() const { return local_prefix_; }
 ...
 private:
  ...
  std::string local_prefix_;   // Local path prefix
  ...
  // For example: "/mnt/nfs_rocksdb/db/0/xxx.sst" -> "/0/xxx.sst"
  std::string ConvertToNFSPath(const std::string& path) const;
  
  // Recursively create a directory (for internal use, need to be called with a lock in place)
  IOStatus CreateDirRecursive(const std::string& nfs_path);
};

// Factory function
// nfs_url: NFS server address, such as "nfs://192.168.1.100/shared/rocksdb"
// local_prefix: Local path prefix, the path used by RocksDB maps to NFS if it starts with this
